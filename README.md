...
course网站使用的还是古董级别的ckeditor 3.6（最新版本已经到ckeditor 5）

一系列flash相关问题也来自这个老旧的富文本编辑框

Course Helper使用wangEditor作为作业编辑器，并实现原编辑器的文件、图片上传功能

**难点**：

1. 文件上传、图片上传
2. wangEditor输出的纯HTML不带有内联样式，直接提交到course中无法看到样式

**解决**：

1. 通过抓包找到原编辑器的图片、文件上传接口，利用模拟post实现文件、图片上传

2. 提交HTML时，附带预先写好的`<style><style/>`，实现样式的注入

   顺带一提，简单操作后也能注入js比如：`<script>alert(\"想不到吧！\")</script>`

   ~~凭我对js简单的了解，比如某管理员查看作业的时候，来上一手 (不是)~~

   ~~然后把设置成什么优秀作业啊，所有同学可见开看，谁来谁中招~~

   总之就是严重的XSS漏洞，完全可以利用管理员暴露的凭证，为所欲为
